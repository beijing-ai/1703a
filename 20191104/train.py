...
import time
import os
import YOLO_v2.yolo.config as cfg

from YOLO_v2.pascal_voc import Pascal_voc
from six.moves import xrange
from YOLO_v2.yolo.yolo_v2 import yolo_v2

class Train(object):
    def __init__(self,yolo,data):
        self.yolo = yolo
        self.data = data
        self.num_calss = len(cfg.CLASSES)
        ...
        self.variable_to_restroe = tf.global_variables()
        self.saver = tf.train.Saver(self.variable_to_restroe)
        self.summary_op = tf.summary.merge_all()
        self.writer = tf.summary.FileWriter(self.output_dir)

        self.global_step = tf.global_variables('global_step',[],initizlizer=tf.constant_initializer(0),trainable=False)
    ...
    def train(self):
        labels_train = self.data.load_labels('trian')
            ...
            if step%self.summary_iter == 0:
                if step%50 == 0:
                    ...
                    log_str = ('{} Epoch:{},Step:{},train_Loss:{:.4f},test_Loss:{:.4f},Remain:{}').format(
                        datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),self.data.epoch,int(step),loss,sum_loss/num,step.remain(step,initial_time)
                    )
                    print(log_str)

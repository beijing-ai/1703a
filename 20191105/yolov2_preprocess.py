...
class Data_preprocess(object):
    ...
    def load_labels(self,model):
        ...
        with open(txtname,'r') as file:
            image_ind = [x.strip() for x in file.readlines()]

        labels = []
        for ind in image_ind:
            label,num = self.load_labels(ind)
            if num == 0:
                continue
    ...
    def load_data(self,index):
        ...
        for obj in objects:
            ...
            class_ind = self.class_to_ind[obj.find('name').test.lower().strip()]

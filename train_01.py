...
import time
import os
import 注释YOLO_v2.yolo.config as cfg

from pascal_voc import Pascal_voc
from six.moves import xrange
from 注释YOLO_v2.yolo.yolo_v2 import yolo_v2
...
class Train(object):
    def __init__(self,yolo,data):
        ...
        self.variable_to_store = tf.global_variables()
        self.saver = tf.train.Saver(self.variable_to_store)
        ...
        for step in xrange(0,self.max_step + 1):
            ...
            if step % self.summary_iter == 0:
                if step % 50 == 0:
                    ...
                    log_str = ('{} Epoch: {}, Step: {}, train_Loss: {:.4f}, test_Loss: {:.4f}, Remain: {}').format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),self.data.epoch,int(step),loss,sum_loss/num,self.remain(step,initial_time))
                    print(log_str)

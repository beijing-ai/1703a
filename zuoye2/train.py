...
class Train(object):
    def __init__(self,yolo,data):
        ...
        self.outout_dir = os.path.join(cfg.DATA_DIR,'output')
        weight_file = os.path.join(self.outout_dir,cfg.WEIGHTS_FILE)

        self.variable_to_restore = tf.global_variables()
        self.saver = tf.train.Saver(self.variable_to_restore)
        self.summary_op = tf.summary.merge_all()
        self.writer = tf.summary.FileWriter(self.outout_dir)

        self.global_step = tf.get_variable('global_step',[],initializer=tf.constant_initializer,trainable=False)
    ...
    def train(self):
        ...
        for step in xrange(0,self.max_step+1):
            ...
            feed_dict = {
                self.yolo.images:images,
                self.yolo.labels:labels
            }

            if step % self.summary_iter == 0:
                if step%50 == 0:
                    summrary_,loss,_ = self.sess.run([self.summary_op,self.yolo.total_loss,self.train_op],feed_dict=feed_dict)
                    sum_loss = 0
                    for i in range(num):
                        images_t,labels_t = self.data.next_batches(labels_test)
                        feed_dict = {
                            self.yolo.images: images_t,
                            self.yolo.labels: labels_t
                        }
                        loss_t = self.sess.run(self.yolo.total_loss,feed_dict=feed_dict)
                        sum_loss+=loss_t
                    log_str = ('{} Epoch: {}, Step: {}, train_Loss: {:.4f}, test_Loss: {:.4f}, Remain: {}').format(
                        ...
                        sum_loss / num, self.remain(step, initial_time))
                    print(log_str)
            ...
            if step%self.saver_iter ==0:
                self.saver.save(self.sess,self.outout_dir+'/yolo_v2.ckpt',global_step=step)

...
class Process(object):
    ...
    def load_data(self,index):
        label = np.zeros([self.cell_szie,self.cell_szie,self.box_per_cell,5+self.num_classes])
        ...
        for obj in objects:
            ...
            boxes = [
                0.5 * (x1 + x2) / self.image_size,
                0.5 * (y1 + y2) / self.image_size,
                np.sqrt((x2-x1)/self.image_size),
                np.sqrt((y2-y1)/self.image_size),
                     ]

            cx = 1.0*boxes[0]*self.cell_szie
            cy = 1.0*boxes[1]*self.cell_szie
    ...
    def next_batch(self,label):
        ...
        labels = np.zeros([self.batch_size,self.cell_szie,self.cell_szie,self.box_per_cell,5+self.num_classes])
        num = 0
